...
export default class MockOrdersService {
  ...
  updateOrderById = jasmine.createSpy('updateOrderById').and.callFake((order: IOrder, items: IProduct[], state?: string) => {
    if (order.id === '123') {
  ...
  getFilteredOrders = jasmine.createSpy('getFilteredOrders').and.returnValue([]);
