...
@Component({
  ...
  template: `
    <div
      class="group relative p-2 mt-8 overflow-hidden rounded-lg shadow-lg bg-gray-200 hover:shadow-xl h-120 flex flex-col"
    >
      <button
        (click)="toggleHeart()"
      ...
      <a
        class="relative block h-3/4 overflow-hidden rounded"
        [routerLink]="['/products', product.id]"
      >
        <div
          class="w-full h-full bg-gray-200 hover:opacity-75 rounded-md overflow-hidden"
        >
          <img
            src="{{ product.image }}"
            alt="{{ product.title }}"
      ...
      <div class="mt-4 flex flex-col gap-2 flex-1">
        <div class="flex justify-between">
          <div>
            ...
            <p
              class="mt-2 inline-block px-2 py-1 text-xs text-gray-500 bg-green-50 rounded-xl"
            >
              {{ product.category }}
            </p>
          </div>
          <p class="text-xl text-gray-900 font-bold">
            {{ product.price | currency: 'BRL' : 'symbol' }}
          </p>
        </div>
        <div class="flex mb-2">
          <app-rating-stars
